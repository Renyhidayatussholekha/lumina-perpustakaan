...
<div class="space-y-10">
    ...
    <!-- ==================================================== -->
    <!-- 1. CINEMATIC HERO SPOTLIGHT (NETFLIX/SPOTIFY STYLE)  -->
    <!-- ==================================================== -->
    <div class="relative rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-br from-space-900 via-space-850 to-space-950 p-6 sm:p-10 shadow-2xl">
        <!-- Ambient Color Glow behind featured cover -->
        <div class="absolute -right-10 -top-10 w-96 h-96 bg-cyan-500/15 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute -left-10 -bottom-10 w-80 h-80 bg-blue-600/10 rounded-full blur-3xl pointer-events-none"></div>

        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <!-- Left Info -->
            <div class="max-w-xl space-y-4 text-left">
                <div class="flex items-center gap-2">
                    <span class="px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs font-bold font-mono uppercase tracking-wider flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-cyan-400 animate-pulse"></span>
                        Buku Pilihan Pekan Ini
                    </span>
                    <span class="text-xs text-slate-400">{{ $featuredBook->category }}</span>
                </div>
                ...
                <div class="p-3.5 rounded-2xl bg-space-950/60 border border-white/10 space-y-1.5 text-xs text-slate-300">
                    <span class="font-bold text-cyan-300 text-[11px] block uppercase tracking-wide">💡 Rangkuman AI Lumina:</span>
                    <p class="italic text-slate-300">"{{ $featuredBook->ai_summary_1 }}"</p>
                </div>
                ...
                <!-- 2 High-Contrast Action Buttons -->
                <div class="flex flex-wrap items-center gap-3 pt-2">
                    <a href="{{ route('lumina.reader', $featuredBook->id) }}" class="bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-300 hover:to-blue-400 text-space-950 font-extrabold px-6 py-3 rounded-xl text-xs sm:text-sm shadow-lg shadow-cyan-500/25 flex items-center gap-2 transition hover:scale-105 active:scale-95">
                        <span>▶</span>
                    ...
                    <button 
                        ...
                        class="bg-white/10 hover:bg-white/15 text-white font-bold px-5 py-3 rounded-xl text-xs sm:text-sm border border-white/15 flex items-center gap-2 transition hover:scale-105 active:scale-95"
                    >
                        <span>🎧</span>
            ...
            <div class="relative shrink-0 hidden md:block group">
                <div class="absolute -inset-2 bg-gradient-to-tr from-cyan-500/30 to-blue-500/30 rounded-2xl blur-xl group-hover:blur-2xl transition duration-500"></div>
                <div class="relative w-48 lg:w-56 aspect-[3/4] rounded-2xl overflow-hidden border border-white/20 cover-shadow transform group-hover:-translate-y-2 transition duration-300">
                    <img src="{{ $featuredBook->cover_url }}" alt="{{ $featuredBook->title }}" class="w-full h-full object-cover">
                    <div class="absolute top-2.5 right-2.5 bg-space-950/80 backdrop-blur px-2.5 py-1 rounded-full text-[10px] font-bold text-amber-400 border border-amber-400/30">
                        ★ {{ $featuredBook->rating }}
                    </div>
                </div>
            </div>
    ...
    <!-- ==================================================== -->
    <!-- 2. SLEEK 3-TAB NAVIGATOR (CLEAN, INTUITIVE & ELEGANT)-->
    <!-- ==================================================== -->
    <div class="flex items-center justify-between border-b border-white/10 pb-4">
        <div class="flex items-center gap-2 p-1 rounded-2xl bg-space-900 border border-white/10">
            <button onclick="switchTab('katalog')" id="tabBtn-katalog" class="tab-btn px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition bg-cyan-500 text-space-950 shadow-md shadow-cyan-500/20 flex items-center gap-2">
                <span>📚</span>
                <span>Rak Buku</span>
            </button>
            <button onclick="switchTab('ai')" id="tabBtn-ai" class="tab-btn px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition text-slate-400 hover:text-white hover:bg-white/5 flex items-center gap-2">
                <span>🤖</span>
                <span>Fitur AI & Tokoh</span>
            </button>
            <button onclick="switchTab('fraksi')" id="tabBtn-fraksi" class="tab-btn px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition text-slate-400 hover:text-white hover:bg-white/5 flex items-center gap-2">
                <span>🏆</span>
        ...
        <div class="hidden sm:flex items-center gap-2 text-xs text-slate-400">
            <span>🦅 Rayhan</span>
            <span class="text-cyan-400 font-bold">• 1.450 XP</span>
        </div>
    </div>
    ...
    <div id="tabContent-katalog" class="tab-content space-y-8">
        ...
        <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
            <button onclick="filterCategory('all', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-bold bg-white text-space-950 transition shrink-0 shadow-sm">
                ✨ Semua Koleksi ({{ $allBooks->count() }})
            </button>
            <button onclick="filterCategory('Sejarah', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-semibold bg-space-900 border border-white/10 text-slate-300 hover:text-white hover:bg-space-800 transition shrink-0">
                🏛️ Sejarah & Tokoh
            </button>
            <button onclick="filterCategory('Sastra', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-semibold bg-space-900 border border-white/10 text-slate-300 hover:text-white hover:bg-space-800 transition shrink-0">
                📖 Sastra & Fiksi
            </button>
            <button onclick="filterCategory('Sains', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-semibold bg-space-900 border border-white/10 text-slate-300 hover:text-white hover:bg-space-800 transition shrink-0">
                🔭 Sains & Teknologi
            </button>
            <button onclick="filterCategory('Pengembangan', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-semibold bg-space-900 border border-white/10 text-slate-300 hover:text-white hover:bg-space-800 transition shrink-0">
                🌿 Stoisisme & Jiwa
            </button>
            <button onclick="filterCategory('Lingkungan', this)" class="cat-pill px-3.5 py-1.5 rounded-full text-xs font-semibold bg-space-900 border border-white/10 text-slate-300 hover:text-white hover:bg-space-800 transition shrink-0">
                🍃 Lingkungan Hidup
            </button>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5" id="booksGrid">
            ...
            <div 
                class="book-card group cursor-pointer flex flex-col justify-between p-3 rounded-2xl glass-card border border-white/10 hover:border-cyan-500/50"
            ...
            >
                <a href="{{ route('lumina.book', $book->id) }}" class="block">
                    <!-- Book Cover -->
                    <div class="relative w-full aspect-[3/4] rounded-xl overflow-hidden bg-space-900 border border-white/10 group-hover:shadow-xl transition duration-300">
                        ...
                        <div class="absolute top-2 left-2 bg-space-950/80 backdrop-blur px-2 py-0.5 rounded-md text-[10px] font-bold text-amber-400 border border-amber-400/20">
                            ★ {{ $book->rating }}
                        </div>
                        <div class="absolute top-2 right-2 bg-space-950/80 backdrop-blur px-2 py-0.5 rounded-md text-[10px] font-bold text-cyan-300 border border-cyan-400/20">
                    ...
                    <div class="pt-3 space-y-1 text-left">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-cyan-400 block line-clamp-1">{{ $book->category }}</span>
                        <h4 class="font-display font-bold text-sm text-white group-hover:text-cyan-300 transition line-clamp-1 leading-snug">
                            {{ $book->title }}
                        </h4>
                ...
                <div class="flex items-center gap-2 pt-3 mt-2 border-t border-white/5">
                    <a href="{{ route('lumina.reader', $book->id) }}" class="flex-1 py-2 rounded-xl bg-white/10 hover:bg-cyan-500 hover:text-space-950 text-white text-xs font-bold text-center transition">
                        Baca
                    </a>
                    <button 
                        onclick="startAudiobook('{{ addslashes($book->title) }}', '{{ addslashes($book->author) }}', '{{ $book->cover_url }}', '{{ addslashes($book->audio_text) }}')" 
                        class="p-2 rounded-xl bg-white/10 hover:bg-space-800 text-slate-300 text-xs transition"
                        title="Dengar Audiobook"
                    >
                        🎧
                    </button>
                    <a href="{{ route('lumina.book', $book->id) }}" class="p-2 rounded-xl bg-white/10 hover:bg-space-800 text-slate-300 text-xs transition" title="Detail">
                        ℹ️
                    </a>
                </div>
            </div>
        ...
        <div class="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-2 text-left">
                    <label class="block text-xs font-bold text-cyan-400 uppercase tracking-wider">1. Mood Membaca</label>
                    <div class="space-y-2">
                        <button type="button" onclick="selectCleanQuiz('mood', 'santai', this)" class="clean-quiz-btn w-full p-3 rounded-xl border border-cyan-500 bg-cyan-500/10 text-cyan-200 text-left text-xs font-semibold transition">
                ...
                <!-- Step 2 -->
                <div class="space-y-2 text-left">
                    <label class="block text-xs font-bold text-cyan-400 uppercase tracking-wider">2. Topik Favorit</label>
                    <div class="space-y-2">
                        <button type="button" onclick="selectCleanQuiz('interest', 'sejarah', this)" class="clean-quiz-btn w-full p-3 rounded-xl border border-cyan-500 bg-cyan-500/10 text-cyan-200 text-left text-xs font-semibold transition">
                ...
                <!-- Step 3 -->
                <div class="space-y-2 text-left">
                    <label class="block text-xs font-bold text-cyan-400 uppercase tracking-wider">3. Waktu Luang</label>
                    <div class="space-y-2">
                        ...
                        <button type="button" onclick="selectCleanQuiz('duration', 30, this)" class="clean-quiz-btn w-full p-3 rounded-xl border border-cyan-500 bg-cyan-500/10 text-cyan-200 text-left text-xs font-semibold transition">
            ...
            <div class="pt-2 flex justify-end">
                <button onclick="submitCleanMatchmaker()" id="submitCleanQuizBtn" class="bg-cyan-500 hover:bg-cyan-400 text-space-950 font-extrabold px-6 py-3 rounded-xl text-xs sm:text-sm shadow-lg shadow-cyan-500/20 transition hover:scale-105 active:scale-95">
                    ✨ Racik Rekomendasi AI
                </button>
            </div>

            <!-- Results -->
            <div id="cleanMatchResults" class="hidden pt-4 border-t border-white/10 space-y-3">
                <div class="p-3.5 bg-cyan-950/40 border border-cyan-500/30 rounded-2xl text-xs text-slate-200">
                    <strong class="text-cyan-400 font-bold">💡 Analisis AI:</strong>
        ...
        <div class="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
                ...
                <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
                    <button onclick="selectCleanCharacter(1, this)" class="char-pill px-3 py-1.5 rounded-xl text-xs font-bold bg-cyan-500 text-space-950 transition">
                        Bung Karno
                    </button>
            ...
            <!-- Chat Window -->
            <div class="border border-white/10 rounded-2xl overflow-hidden bg-space-950/70">
                <div class="p-4 bg-space-900 border-b border-white/10 flex items-center gap-3">
                    <img id="activeCharAvatar" src="https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&w=120&q=80" class="w-10 h-10 rounded-xl object-cover border border-cyan-500/30">
                    <div>
                        <h4 id="activeCharName" class="font-bold text-sm text-white">Ir. Soekarno (Bung Karno)</h4>
                        <p id="activeCharRole" class="text-xs text-slate-400">Proklamator & Presiden RI Pertama</p>
                    </div>
                </div>

                <div id="cleanChatHistory" class="p-4 space-y-3 max-h-72 overflow-y-auto text-xs sm:text-sm">
                    <div class="flex gap-3 items-start">
                        <div class="bg-space-900 border border-white/10 rounded-2xl p-3.5 max-w-[85%] text-slate-200">
                ...
                <div class="p-3 bg-space-900 border-t border-white/10 flex items-center gap-2">
                    <input 
                        type="text" 
                        id="cleanChatInput" 
                        placeholder="Ketik pertanyaan untuk tokoh ini..." 
                        class="flex-1 bg-space-950 border border-white/10 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-400"
                        onkeydown="if(event.key === 'Enter') sendCleanChatMessage()"
                    >
                    <button onclick="sendCleanChatMessage()" class="bg-cyan-500 hover:bg-cyan-400 text-space-950 font-bold px-4 py-2.5 rounded-xl text-xs sm:text-sm transition">
                        Kirim
                    </button>
                </div>
            </div>
    ...
    <div id="tabContent-fraksi" class="tab-content hidden space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="p-5 rounded-3xl glass-card border border-white/10 space-y-3 {{ $h->name === $student['house'] ? 'border-cyan-400/60 shadow-lg shadow-cyan-500/10' : '' }}">
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-bold text-xs text-cyan-400 font-mono">#{{ $hIndex + 1 }}</span>
                </div>
            ...
            <div class="divide-y divide-white/10 text-xs sm:text-sm">
                <div class="py-3 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Siti" class="w-8 h-8 rounded-full bg-space-800 border border-slate-700">
                        <div>
                            <span class="font-bold text-white block">Siti Nurhaliza</span>
                            <span class="text-[11px] text-slate-400">12 IPS 1 • 🐉 Komodo Wira</span>
                        </div>
                    </div>
                    <span class="font-mono text-cyan-400 font-bold">1.840 Menit</span>
                </div>

                <div class="py-3 flex items-center justify-between bg-cyan-500/10 -mx-6 px-6 border-l-4 border-cyan-400">
                    <div class="flex items-center gap-3">
                        <span class="text-xl">🥈</span>
                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Rayhan" class="w-8 h-8 rounded-full bg-space-800 border border-cyan-400">
                        <div>
                            <span class="font-bold text-white block">Rayhan Alfarizi (Kamu)</span>
                            <span class="text-[11px] text-cyan-300">11 IPA 2 • 🦅 Garuda Cendekia</span>
                        </div>
                    </div>
                    <span class="font-mono text-cyan-300 font-bold">1.045 Menit</span>
                </div>

                <div class="py-3 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <span class="text-xl">🥉</span>
                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Aditya" class="w-8 h-8 rounded-full bg-space-800 border border-slate-700">
                        <div>
                    ...
                    <span class="font-mono text-cyan-400 font-bold">980 Menit</span>
                </div>
            </div>

            <div class="pt-2 text-center">
                <a href="{{ route('lumina.gamification') }}" class="text-xs text-cyan-400 font-bold hover:underline">
...
<script>
    function switchTab(tabName) {
        ...
        document.querySelectorAll('.tab-btn').forEach(b => {
            b.classList.remove('bg-cyan-500', 'text-space-950', 'shadow-md', 'shadow-cyan-500/20');
        ...
        if (targetContent && targetBtn) {
            ...
            targetBtn.classList.add('bg-cyan-500', 'text-space-950', 'shadow-md', 'shadow-cyan-500/20');
        }
    }

    function filterCategory(cat, btn) {
        document.querySelectorAll('.cat-pill').forEach(b => {
            b.classList.remove('bg-white', 'text-space-950');
            b.classList.add('bg-space-900', 'text-slate-300');
        });
        btn.classList.remove('bg-space-900', 'text-slate-300');
        btn.classList.add('bg-white', 'text-space-950');
    ...
    function selectCleanQuiz(type, val, btn) {
        ...
        btn.parentElement.querySelectorAll('button').forEach(b => {
            b.classList.remove('border-cyan-500', 'bg-cyan-500/10', 'text-cyan-200');
            b.classList.add('border-white/10', 'text-slate-300');
        });
        btn.classList.add('border-cyan-500', 'bg-cyan-500/10', 'text-cyan-200');
    ...
    async function submitCleanMatchmaker() {
        ...
        try {
            ...
            data.matched_books.forEach(b => {
                ...
                card.className = 'p-3 rounded-2xl border border-white/10 bg-space-900 flex items-center justify-between gap-3';
                card.innerHTML = `
                    <div class="flex items-center gap-3 min-w-0">
                        <img src="${b.cover_url}" class="w-10 h-14 rounded-lg object-cover shrink-0">
                        <div class="min-w-0">
                            <span class="text-[10px] text-cyan-400 font-bold">${b.match_score}% Match</span>
                            <h5 class="text-xs font-bold text-white truncate">${b.title}</h5>
                            <p class="text-[11px] text-slate-400 truncate">${b.author}</p>
                        </div>
                    </div>
                    <a href="/buku/${b.id}" class="px-3 py-1.5 bg-cyan-500 hover:bg-cyan-400 text-space-950 font-bold rounded-xl text-xs shrink-0 transition">Buka</a>
                `;
    ...
    function selectCleanCharacter(id, btn) {
        ...
        document.querySelectorAll('.char-pill').forEach(b => {
            b.classList.remove('bg-cyan-500', 'text-space-950', 'font-bold');
            b.classList.add('bg-white/10', 'text-slate-300');
        });
        btn.classList.remove('bg-white/10', 'text-slate-300');
        btn.classList.add('bg-cyan-500', 'text-space-950', 'font-bold');
    ...
    async function sendCleanChatMessage() {
        ...
        userDiv.innerHTML = `<div class="bg-cyan-500 text-space-950 font-semibold rounded-2xl p-3 max-w-[85%]">${text}</div>`;
        ...
        try {
            ...
            botDiv.innerHTML = `<div class="bg-space-900 border border-white/10 text-slate-200 rounded-2xl p-3 max-w-[85%] leading-relaxed">${data.reply}</div>`;
